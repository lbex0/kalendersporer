...
export default class SourceFileLinter {
    ...
    #extractXmlFromJs(callExpression, apiName) {
        ...
        const traceMap = sourceMapJson ?
            new TraceMap(JSON.parse(sourceMapJson)) :
            undefined;
    ...
    #analyzeCreateComponentCall(node) {
        if (!node.arguments.length || !ts.isObjectLiteralExpression(node.arguments[0])) {
            return;
        }
        const asyncNode = getPropertyAssignmentInObjectLiteralExpression("async", node.arguments[0]);
    ...
    #analyzeOdataModelV2CreateEntry(node) {
        if (!node.arguments.length || node.arguments.length < 2 || !ts.isObjectLiteralExpression(node.arguments[1])) {
            return;
        }
        const [batchGroupId, properties] = getPropertyAssignmentsInObjectLiteralExpression(["batchGroupId", "properties"], node.arguments[1]);
        if (batchGroupId) {
            this.#reporter.addMessage(MESSAGE.PARTIALLY_DEPRECATED_ODATA_MODEL_V2_CREATE_ENTRY, null, { node: batchGroupId });
        }
        if (properties && ts.isArrayLiteralExpression(properties.initializer)) {
            this.#reporter.addMessage(MESSAGE.PARTIALLY_DEPRECATED_ODATA_MODEL_V2_CREATE_ENTRY_PROPERTIES_ARRAY, null, { node: properties });
        }
    }
    #analyzeJsonModelLoadDataCall(node) {
    ...
    #analyzeMobileInit(node) {
        if (!node.arguments.length || !ts.isObjectLiteralExpression(node.arguments[0])) {
            return;
        }
        const [homeIconArg, homeIconPrecomposedArg] = getPropertyAssignmentsInObjectLiteralExpression(["homeIcon", "homeIconPrecomposed"], node.arguments[0]);
        if (homeIconArg) {
    ...
    #analyzeNewCoreRouter(node) {
        if (!node.arguments || node.arguments.length < 2 || !ts.isObjectLiteralExpression(node.arguments[1])) {
            return;
        }
        const asyncProb = getPropertyAssignmentInObjectLiteralExpression("async", node.arguments[1]);
    ...
    #analyzeFragmentLoad(node, symbolName) {
    ...
    #analyzeNewAndApplySettings(node) {
    ...
    #analyzePropertyBindings(node, propNames) {
        node?.properties.forEach((prop) => {
            ...
            let propertyField;
            if (ts.isObjectLiteralExpression(prop.initializer)) {
                propertyField = getPropertyAssignmentsInObjectLiteralExpression(propNames, prop.initializer)[0];
            }
    ...
    #analyzePropertyStringBindings(node) {
        if (ts.isStringLiteralLike(node.initializer) &&
            node.initializer.text.startsWith("{") && node.initializer.text.endsWith("}")) {
            const imports = this.sourceFile.statements
                .filter((stmnt) => stmnt.kind === ts.SyntaxKind.ImportDeclaration)
                .map((importNode) => {
                return {
                    moduleName: importNode.moduleSpecifier.text,
    ...
    */
    #isPropertyBinding(node, propNames) {
        const controlAmbientModule = getSymbolModuleDeclaration(this.checker.getTypeAtLocation(node).symbol);
        let classArg;
        if (controlAmbientModule?.body && ts.isModuleBlock(controlAmbientModule.body)) {
            classArg = controlAmbientModule.body.statements
                ?.find((stmnt) => stmnt.kind === ts.SyntaxKind.ClassDeclaration)
        ...
        const argProperty = constructorArgType?.getProperties()
            .find((p) => propNames.includes(p.name));
    ...
    #analyzeBindingPropertyNode(node) {
        ...
        const hasModuleImported = this.sourceFile.statements.filter(ts.isImportDeclaration)
            .some((importNode) => ts.isStringLiteralLike(importNode.moduleSpecifier) &&
    ...
    getDeprecationInfoForAccess(node) {
        let symbol;
    ...
    analyzePropertyAccessExpressionForDeprecation(node) {
        ...
        if (this.isSymbolOfJquerySapType(deprecationInfo.symbol)) {
            ...
            this.#reporter.addMessage(MESSAGE.DEPRECATED_PROPERTY, {
                propertyName: deprecationInfo.symbol.escapedName,
                namespace,
                details: deprecationInfo.messageDetails,
            }, {
                node,
    ...
    handleCallExpressionUnknownType(nodeType, node) {
        ...
        if (typeString === "any" && !ts.isImportDeclaration(node.parent.parent)) {
            this.#reporter.addCoverageInfo({
                ...
                message: `Unable to analyze this method call because the type of identifier` +
                    `${identifier ? " \"" + identifier.text + "\"" : ""} in "${node.getText()}" ` +
                    `could not be determined`,
            });
        }
    }
    analyzeExportedValuesByLib(node) {
        ...
        for (const moduleChunk of [...namespace, varName]) {
            ...
            const exportMap = libExports.get(moduleChunk);
    ...
    isAllowedPropertyAccess(node) {
        if (!ts.isIdentifier(node.expression)) {
            // TODO: Fixme if this happens
            throw new Error(`Unhandled PropertyAccessExpression expression syntax: ${ts.SyntaxKind[node.expression.kind]}`);
        }
    ...
    analyzeImportDeclaration(importDeclarationNode) {
        ...
        const defaultExportSymbol = symbol.exports?.get("default");
        ...
        if (importClause?.namedBindings && ts.isNamedImports(importClause.namedBindings)) {
            ...
            for (const namedImportElement of importClause.namedBindings.elements) {
                ...
                const namedImportSymbol = symbol.exports?.get(importName);
        ...
        if (this.isSymbolOfPseudoModuleType(symbol)) {
            ...
            if (isEnum) {
                this.#reporter.addMessage(MESSAGE.NO_DIRECT_ENUM_ACCESS, { moduleName }, { node: moduleSpecifierNode });
            }
            else { // Data Type
                this.#reporter.addMessage(MESSAGE.NO_DIRECT_DATATYPE_ACCESS, { moduleName }, { node: moduleSpecifierNode });
            }
        }
    }
    isSymbolOfUi5Type(symbol) {
        return this.checkSymbolDeclarationSourceFile(symbol, isSourceFileOfUi5Type);
    }
    isSymbolOfUi5OrThirdPartyType(symbol) {
        return this.checkSymbolDeclarationSourceFile(symbol, isSourceFileOfUi5OrThirdPartyType);
    }
    isSymbolOfJquerySapType(symbol) {
        return this.checkSymbolDeclarationSourceFile(symbol, isSourceFileOfJquerySapType);
    }
    isSymbolOfPseudoModuleType(symbol) {
        return this.checkSymbolDeclarationSourceFile(symbol, isSourceFileOfPseudoModuleType);
    }
    checkSymbolDeclarationSourceFile(symbol, checkFunction) {
    ...
    analyzeTestsuiteThemeProperty(node) {
        // Check if the node is part of a testsuite config file by its file name.
        if (!VALID_TESTSUITE.test(node.getSourceFile().fileName))
            return;
...
//# sourceMappingURL=SourceFileLinter.js.map